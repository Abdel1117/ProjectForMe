<main class="Maincontainer" id="Maincontainer">
            <div class="body-container" id="body-container">
                <div class="promo-Panel" id="promo-panel1">
                    <h2 class="title" id="title">TEL</h2>
                    <div class="wraper" id="wrapper">
                        <img class="c" src="https://images.assetsdelivery.com/compings_v2/abolins/abolins1611/abolins161100041.jpg" alt="">
                    </div>

                    <div id="wrapper2">
                        <h3 class="caption">TEL</h3>
                        <div class="Div-para">
                            <p class="Para-info">Contactez moi par télèphone au:<strong>Tel:06 00 00 00 00 </strong></p>
                        </div>

                    </div>

                </div>

                
                <div class="promo-Panel" id="promo-panel2">
                    <h2 class="title" id="title">Email</h2>
                    <div class="wraper" id="wraper">

                        <img class="c" src="data:image/png;base64," alt="">
                    </div>
                    <div id="wrapper2">
                        <h3 class="caption">Email</h3>
                        <div class="Div-para">
                            <p class="Para-info">Contactez moi par email sur cette adresse: <strong>Abderahmane.adjali@live.fr<strong></p>
                        </div>

                    </div>

                </div>
                
                
                
                <div class="promo-Panel" id="promo-panel3">               
                    <h2 class="title" id="title">Linkedin</h2>
                
                    <div class="wraper" id="wraper">
                    <img id="Java-img" class="c" src="data:image/png;base64," alt="">
                    </div>
                <div id="wrapper2">
                        <h3 class="caption">Linkedin</h3>
                        <div class="Div-para">
                            <p class="Para-info">Contactez moi sur Linkedin: <a href="https://www.linkedin.com/feed/"><strong>ICI</strong></a></p>
                        </div>
                    </div>
                
                </div>        


                
            </div>



        </main>